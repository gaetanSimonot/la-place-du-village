...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  const { data: comment } = await supabaseAdmin
    .from('etablissement_comments').select('user_id').eq('id', commentId).maybeSingle()
  ...
  const { data, error } = await supabaseAdmin
    .from('etablissement_comments').update({ content: content.trim() }).eq('id', commentId)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  const { error } = await supabaseAdmin.from('etablissement_comments').delete().eq('id', commentId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
