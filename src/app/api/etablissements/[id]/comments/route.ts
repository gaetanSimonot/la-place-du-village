...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabaseAdmin
    .from('etablissement_comments')
    .insert({ etablissement_id: id, user_id: user.id, content: content.trim(), parent_id: null })
    .select('id, user_id, content, parent_id, created_at')
    .single()
  ...
  const { data: profile } = await supabaseAdmin
    .from('profiles').select('user_id, display_name, avatar_url').eq('user_id', user.id).maybeSingle()

  const { data: etab } = await supabaseAdmin
    .from('etablissements').select('user_id').eq('id', id).maybeSingle()
  if (etab?.user_id && etab.user_id !== user.id) {
    await supabaseAdmin.from('notifications').insert({
      user_id: etab.user_id,
      type: 'commentaire',
      actor_id: user.id,
