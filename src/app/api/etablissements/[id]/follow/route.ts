...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (existing) {
    const { error } = await supabaseAdmin.from('etablissement_followers')
      .delete().eq('user_id', user.id).eq('etablissement_id', id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ following: false })
  }

  const { error } = await supabaseAdmin.from('etablissement_followers')
    .insert({ user_id: user.id, etablissement_id: id })
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  const { data: etab } = await supabaseAdmin
    .from('etablissements').select('user_id, nom').eq('id', id).maybeSingle()
