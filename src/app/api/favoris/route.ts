...
export async function GET(req: NextRequest) {
  ...
  const [producersRes, etabsRes, promosRes] = await Promise.all([
    allPIds.length
    ...
    allEIds.length
      ...
      : Promise.resolve({ data: [] }),
    promoFavIds.length
      ? supabaseAdmin.from('promotions').select('id, title, image_url, etablissement_id, etablissements(nom)').in('id', promoFavIds)
      : Promise.resolve({ data: [] }),
  ])
  ...
  const promosById: Record<string, { id: string; title: string; image: string | null; etabNom: string | null }> = {}
  for (const p of (promosRes.data ?? []) as Array<{
    id: string; title: string | null; image_url: string | null;
    etablissements: { nom: string } | { nom: string }[] | null;
  }>) {
    const etab = Array.isArray(p.etablissements) ? p.etablissements[0] : p.etablissements
    promosById[p.id] = { id: p.id, title: p.title ?? 'Promotion', image: p.image_url ?? null, etabNom: etab?.nom ?? null }
  }
