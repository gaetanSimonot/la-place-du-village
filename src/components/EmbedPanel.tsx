...
/**
 * EmbedPanel : version INLINE (sans portal/modal) de EmbedPicker pour
 * utilisation dans un panneau bottom (chat amis).
 *
 * Mêmes 3 modes que EmbedPicker :
 *  1. Vue ROOT : grille catégories cliquables
 *  2. Vue CATEGORIE : liste items du kind sélectionné, scrollable
 *  3. Vue RECHERCHE : multi-cat
 *
 * À la différence d'EmbedPicker, ce composant prend la hauteur disponible
 * du parent (h-full) et ne gère pas son propre fond / drag handle. Le
 * parent panel s'occupe du chrome.
 */
...
export default function EmbedPanel({ onSelect, onClose }: Props) {
  ...
  // Search : 3 modes (libre / mono-cat / browse)
  ...
  // Groupé par kind pour recherche multi-cat
  ...
  return (
    <div className="flex h-full w-full flex-col bg-white">
      {/* Header : back si dans une cat, search input, close */}
      <div className="flex shrink-0 items-center gap-2 border-b border-bordSoft px-3 py-2">
        {selectedKind !== null && (
          <button
            type="button"
        ...
        <div
          className="flex flex-1 items-center gap-2.5 rounded-[12px] border bg-white px-3 py-2"
          style={{ borderColor: '#E8E0D4' }}
        >
          <svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke="#7A6A5A" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
          ...
          <input
            ...
            placeholder={
              selectedKind !== null
                ? `Dans ${KIND_LABEL[selectedKind].toLowerCase()}…`
        ...
        <button
          ...
          onClick={onClose}
          aria-label="Fermer"
          className="flex h-9 w-9 shrink-0 items-center justify-center bg-transparent text-texte-doux"
        >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3" style={{ minHeight: 0 }}>
            ...
            {ORDERED_KINDS.map(kind => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-center text-[11px] font-bold leading-[1.15]" style={{ color: '#1A1209' }}>
                    {KIND_LABEL[kind]}
                  </div>
                </button>
              )
        ...
        {/* Résultats multi-cat (recherche depuis racine) */}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[13px] font-extrabold text-texte">{item.title}</div>
        {item.subtitle && (
