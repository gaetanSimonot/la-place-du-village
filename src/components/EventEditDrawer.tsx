...
export default function EventEditDrawer({ evenementId, initialData, initialImage, onClose, onSaved }: Props) {
  ...
  /** Valide les champs critiques avant POST. Renvoie true si tout est OK. */
  const validate = (): boolean => {
    const errs: Record<string, string> = {}
    if (!titre.trim()) errs.titre = 'Le titre est requis.'
    const hasLieu = !!(lieuNom.trim() || commune.trim() || adresse.trim())
    if (!dateDebut && !hasLieu) {
      errs.dateDebut = 'Indique au moins une date ou un lieu.'
      errs.lieu      = 'Indique au moins une date ou un lieu.'
    }
    setFieldErrors(errs)
          ...
          <div>
            ...
            <div className="flex items-center gap-2 rounded-xl bg-white px-3.5 py-2.5" style={{ border: `1px solid ${fieldErrors.dateDebut ? '#E53935' : '#F0EAE0'}` }}>
              ...
              <input type="date" value={dateDebut}
                ...
                className="w-full border-none bg-transparent text-[13px] text-texte outline-none" />
            </div>
          ...
          <div className="rounded-xl bg-white p-3" style={{ border: `1px solid ${fieldErrors.lieu ? '#E53935' : '#F0EAE0'}` }}>
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <input type="text" value={lieuNom}
                  onChange={e => { setLieuNom(e.target.value); if (fieldErrors.lieu) setFieldErrors(p => ({ ...p, lieu: '', dateDebut: '' })) }}
                  placeholder="Nom du lieu" className="w-full border-none bg-transparent text-[14px] font-bold text-texte outline-none" />
                <input type="text" value={[adresse, commune].filter(Boolean).join(', ')}
                  onChange={e => {
                    const parts = e.target.value.split(',')
                    setAdresse(parts[0]?.trim() ?? '')
                    setCommune(parts.slice(1).join(',').trim())
                    if (fieldErrors.lieu) setFieldErrors(p => ({ ...p, lieu: '', dateDebut: '' }))
                  }}
                  placeholder="Adresse, commune" className="w-full border-none bg-transparent text-[12px] text-texte-doux outline-none" />
              </div>
            </div>
          </div>
          {fieldErrors.lieu && <p className="m-0 mt-1 text-[11px] text-[#E53935]">{fieldErrors.lieu}</p>}
