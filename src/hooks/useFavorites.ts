...
/**
 * Favoris événement avec stockage hybride :
 *  - User connecté : DB (`event_favorites` via /api/profile/favorites + /api/evenements/[id]/favorite)
 *  - Anonyme       : localStorage `pdv-favoris`
 *  - Au login      : import auto des favoris locaux puis clear localStorage
 *
 * API du hook inchangée : `{ favIds, toggle, isFav }`.
 *
 * Updates optimistes : on bouge l'état local instantanément puis l'API
 * confirme/rollback en cas d'erreur.
 */
    ...
    setFavIds(prev => {
      ...
      if (!user) {
        ...
        ;(async () => {
          ...
          const token = session?.access_token
          if (!token) return
          const res = await fetch(`/api/evenements/${id}/favorite`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${token}` },
          }).catch(() => null)
          if (!res || !res.ok) {
