...
export async function render({ d, W, H, fx, sources }) {
  ...
  const dateBlock = Box({ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 2.2 * u },
    ...
    Box({ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: colH, paddingTop: 0.6 * u, paddingBottom: 0.6 * u },
      Text({ fontFamily: 'Poppins', fontWeight: 700, fontSize: 2.5 * u, letterSpacing: 5, textTransform: 'uppercase', color: P.muted }, d.weekday),
      Text({ fontFamily: 'Archivo Black', fontSize: 6.8 * u, lineHeight: 0.9, textTransform: 'uppercase', color: P.accent, letterSpacing: -0.5 }, d.month),
      Text({ fontFamily: 'Poppins', fontWeight: 700, fontSize: 2.9 * u, letterSpacing: 6, color: P.ink }, d.year)));
  ...
  return Box(
    ...
    Box({ display: 'flex', flexDirection: 'column', padding: M },
      Box({ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
        Box({ display: 'flex', paddingTop: 1.1 * u, paddingBottom: 1.1 * u, paddingLeft: 2 * u, paddingRight: 2 * u, borderRadius: 999, border: `2px solid ${P.ink}`, fontFamily: 'Poppins', fontWeight: 700, fontSize: 1.9 * u, letterSpacing: 3, textTransform: 'uppercase', color: P.ink }, d.tag),
        logoEl),
      Text({ fontFamily: 'Poppins', fontWeight: 800, fontSize: 2.5 * u, letterSpacing: 2.5, textTransform: 'uppercase', color: P.accent, marginTop: 4.4 * u }, [d.title, d.catLabel].filter(Boolean).join('  —  ')),
      Box({ display: 'flex', marginTop: 1.8 * u }, dateBlock),
      Box({ display: 'flex', width: 9 * u, height: 0.55 * u, background: P.accent, marginTop: 2.8 * u }),
      d.description ? Text({ fontFamily: 'Poppins', fontWeight: 500, fontSize: 2.5 * u, lineHeight: 1.5, color: P.muted, maxWidth: '80%', marginTop: 1.8 * u }, d.description) : null),
    photo
